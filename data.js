		...
		res.on("end", function(){
			...
			try{
				...
				try{
					...
					client.query("select * from z4r5jk12_data_admin where type=? and number=? limit 1", [data.type, data.number], function(err, rows){
						if(err){
							console.log('数据库错误');
...
function liRunData(data, conf){
	...
	getLiRunLv();
	var client=createMySQLClient();
	client.query("select * from z4r5jk12_bets where type=? and actionNo=? and isDelete=0 and lotteryNo=''", [data.type, data.number], function(err, bets){
		if(err){
			log("读取投注出错："+err);
		}else{
